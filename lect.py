...
TIME_PED = 1 # a new pedestrian enters each 2s
TIME_IN_BRIDGE_CARS = (1, 0.15) # normal 1s, 0.5s
TIME_IN_BRIDGE_PEDESTRIAN = (3, 0.5) # normal 10s, 3s
...
class Monitor():
    def __init__(self):
        ...
        #Estos valores marcaran cuantos peatones/coches hay esperando
        self.peatEsper=Value('i', 0)
        self.cochEsperS=Value('i', 0)
        self.cochEsperN=Value('i', 0)


    def pasar_sur(self):
        return self.peat.value == 0 and self.cochesN.value == 0 and \
            ( (self.peatEsper.value+self.cochEsperN.value) <3 or self.turn.value ==0)
            
    def pasar_nort(self):
        return self.peat.value == 0 and self.cochesS.value == 0 and \
            ( (self.peatEsper.value + self.cochEsperS.value) <3 or self.turn.value ==2)
    def pasar_peat(self):
        return self.cochesN.value==0 and self.cochesS.value ==0 and \
            ( (self.cochEsperN.value + self.cochEsperS.value) <3 or self.turn.value ==1)
    ...
    def leaves_car(self, direction: int) -> None:
        ...
        if direction==1:
            self.cochesS.value -=1
            #Ahora que toque a los coches Norte
            self.turn.value = 2
            self.pasar_peatUP.notify_all()
            self.pasar_nortUP.notify_all()
        if direction==0:
            self.cochesN.value -=1
            #Ahora que toque a los peatones
            self.turn.value = 1
            self.pasar_peatUP.notify_all()
    ...
    def leaves_pedestrian(self) -> None:
        ...
        self.patata.value += 1
        self.peat.value -=1
        #Ahora que toque a los coches Sur
        self.turn.value = 0
